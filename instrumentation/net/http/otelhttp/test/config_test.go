...
func TestBasicAttributeFilter(t *testing.T) {
	rr := httptest.NewRecorder()

	spanRecorder := tracetest.NewSpanRecorder()
	provider := trace.NewTracerProvider(trace.WithSpanProcessor(spanRecorder))
	filteredKey := attribute.Key("http.method")
	...
	h := otelhttp.NewHandler(
		...
		otelhttp.WithTracerProvider(provider),
		otelhttp.WithAttributeFilter(func(a attribute.KeyValue) bool {
			return a.Key != filteredKey
		}),
	)
	...
	if assert.Len(t, spans, 1) {
		for _, a := range spans[0].Attributes() {
			assert.NotEqual(t, filteredKey, a.Key)
		}
	}
}
